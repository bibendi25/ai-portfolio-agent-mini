...
export async function POST(req){
  try{
    ...
    const mode = body.mode || url.searchParams.get('mode') || 'ask';

    // Load manifest (no fs at runtime)
    ...
    if(!man.ok) {
      return NextResponse.json({ ok:false, error:`No /docs/manifest.json (${man.error})`, hits:[], note:'', md:'' });
    }
    ...
    if (mode === 'cover') {
      ...
      const bullets = (top.slice(0,3).length
        ? top.slice(0,3).map(r=>`- ${r} → evidence in ${projNames}`).join('\n')
